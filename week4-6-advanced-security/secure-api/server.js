...
if (!JWT_SECRET) {
  console.error('[FATAL] JWT_SECRET environment variable not set. See .env.example');
  process.exit(1);
}

const API_KEY = process.env.API_KEY;
if (!API_KEY) {
  console.error('[FATAL] API_KEY environment variable not set. See .env.example');
  process.exit(1);
}
...
// 6. CSRF Protection
// Note: csurf is deprecated. For production use, replace with csrf-csrf package.
...
// ============================================
// DATABASE MOCK
// Note: This is a demo mock. In production, replace with
// a real database and use bcrypt.compare() for password checks.
// ============================================
const users = [
  {
    id: 1,
    email: 'admin@juice-sh.op',
    password: '$2b$10$YKU5qVR5RqFqZ5QZ5QZ5QO', // placeholder hash - login uses mock check below
    role: 'admin'
  }
];
...
app.post('/api/auth/login', async (req, res) => {
  ...
  // Demo-only mock check. In production: use bcrypt.compare(password, user.password)
...
// SQL Injection SAFE endpoint
app.get('/api/products/search', (req, res) => {
  const { q } = req.query;
  // SAFE: parameterized query simulation
  // In production: db.query('SELECT * FROM products WHERE name LIKE ?', [`%${q}%`])
  res.json({
...
app.listen(PORT, () => {
  ...
  console.log('  CSRF Protection (note: csurf deprecated - use csrf-csrf in production)');
